...
const $commentCnt = $commentWrap.find('.comment-cnt');
...
const $checkMsg = $('#guest-book #check-msg');
const $loadMoreButton = $('#scroll').find('.btn-load-more');

// 늘어나는 코멘트에 따라 .wrap 높이와 body 높이 조절
const autoScroll = () => {
    $('.wrap').animate({ 
...
commentData.forEach(item => {
    ...
    $commentWrap.append(`${dataContentEdit ? comment_edit_html : comment_html}`);
...
$commentMakeBtn.click(async function () {
    ...
    let $commentContent = $('#gb-content').val().replace(/(?:\r\n|\r|\n)/g, "<br>");
    ...
    if ($inputPwd.val() !== password) {
        ...
        $('.gb-comment').each(function() {
            if ($(this).data('id') == currentCommentId) {
                ...
                <textarea name="gb-content-confirm" id="gb-content-confirm" cols="65" rows="10" required>${dataEditContent ? dataEditContent : dataContent}</textarea>
